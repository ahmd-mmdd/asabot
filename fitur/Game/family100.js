...
async function message(sock, m, store) {
    ...
    if(!isset(sock.game[id])){
        const {data: {Barqah: {soal,jawaban}}} = await sock.sendRequest(m).get(`${baseURL}/api/minigame/family100?apikey=${config.apikey}`)
        sock.game[id] = {soal, jawaban, money: Math.floor(Math.random() * 30)}
