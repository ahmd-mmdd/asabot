...
async function message(sock, m, store) {
    ...
    const orangnya = await sock.groupRequestParticipantsList(id);
    if(orangnya.length == 0) return nyarios(`tidak ada yang masuk di grup`);
    const listIDMasuk = orangnya.map(v => v.jid);
    sock.groupRequestParticipantsUpdate(id,listIDMasuk,"approve");
    nyarios(`${listIDMasuk.map(v => `@${v.split("@")}`).join(', ')} telah di approve admin oleh @${nomor} dari *${subject}*`);
